...
class Knn_infer:
    def __init__(self, arcface_backbone,knn_model_path,labels_path):
        ...
        self.my_bucket = 'ludex-ml-testing'
        self.s3client = boto3.client('s3')
       
        # KNN model
        self.myfile1 = knn_model_path
        print("key*******",knn_model_path)
        response1 =  self.s3client.get_object(Bucket = self.my_bucket, Key=self.myfile1)
        body1 = response1['Body'] 
        self.knn = pickle.loads(body1.read())
            
        # labels    
        self.myfile2 = labels_path
        response2 =  self.s3client.get_object(Bucket = self.my_bucket, Key=self.myfile2)
        body2 = response2['Body'] 
        dict1  = json.loads(body2.read()) 
...
#         sets = list1[2]
    ...
    def inference(self,data_dir,outdir):
        ...
        for i in range(0,len(classes)):
            ...
            #val = count/total
            #if val<0.5:
            d.append({"class_name":classes[i],"total":total,"number":count})
        print(str(data_dir.split("/")[-1]) + "Accuracy : ",accuracy_score(gt_list,pred_list)*100)
        df = pd.DataFrame(d)
