...
class KNN:
    def __init__(self,arcface_backbone,out_path = "/home/ec2-user/SageMaker/all_knn/"):
        ...
        self.bucket_name = "ludex-ml-testing"
...
#             json.dump(final_dict, outfile)
        ...
        key = 'knn-bv/'+str(input_dir.split("/")[len(input_dir.split("/"))-2])+"/"+str(input_dir.split("/")[len(input_dir.split("/"))-1])+"/"+"KNN_"+str(input_dir.split("/")[-1])+".pickle"
        pickle_byte_obj = pickle.dumps(out)
        print("key  ",key)
        self.conn.Object(self.bucket_name,key).put(Body=pickle_byte_obj)
        
        key2 = 'knn-bv/'+str(input_dir.split("/")[len(input_dir.split("/"))-2])+"/"+str(input_dir.split("/")[len(input_dir.split("/"))-1])+"/"+"all_labels_"+str(input_dir.split("/")[-1])+".json"
        # json_obj = json.dump(final_dict,key2)
        self.conn.Object(self.bucket_name,key2).put(Body=json.dumps(final_dict))
